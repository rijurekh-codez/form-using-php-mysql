...
<body class=" flex bg-green-100 justify-center items-center h-screen ">
  ...
  <form class=" bg-indigo-200 rounded-lg p-4 w-100" action="signin.php" method="post">
    <h1 class="text-xl mb-4 font-medium">Sign in</h2>
      <div class="mb-5 pr-4">
        <input type="username" name="username" id="username" class=" border border-gray-300  text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-200 dark:border-gray-600   dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Enter username" required />
      </div>
      <div class="mb-5 pr-4">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $password = (int) mysqli_real_escape_string($conn, $_POST['password']);
